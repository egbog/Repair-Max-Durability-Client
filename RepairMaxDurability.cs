	...
	public class RepairMaxDurability : ModulePatch
	{
		...
		public static bool Postfix(ref ItemView __instance, ItemContextClass dragItemContext, PointerEventData eventData)
		{
			...
			if (eventData.pointerEnter != null)
			{
				...
				// verify the item is owned by the player and not in a trader's assort
				if (!CheckOwner(targetItem))
				{
					targetItem = null;
					log.LogWarning("UNABLE TO REPAIR ITEMS IN TRADER'S INVENTORY!");
				}

				// verify we're not dragging a repair kit from a trader's assort
				if (!CheckOwner(dragItemContext.Item))
				{
					dragItemContext = null;
					log.LogWarning("UNABLE TO REPAIR USING ITEMS IN TRADER'S INVENTORY!");
				}
			} 

			// only do work when our item is dragged AND dragged onto another item
			// make sure the item being dragged is the repair kit
			if (dragItemContext != null && targetItem != null && CheckName(dragItemContext.Item))
			{
				// make sure it's an item that can actually be repaired ie. weapon
				// must contain a RepairableComponent
				if (targetItem.TryGetItemComponent<RepairableComponent>(out RepairableComponent repairableComponent))
				{
				...
				else // targetItem is something other than a gun
				{
                    log.LogWarning("REPAIR FAILED: NO REPAIRABLECOMPONENT DETECTED");
                    dragItemContext.DragCancelled();
                }
